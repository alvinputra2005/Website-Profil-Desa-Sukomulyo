    ...
    public function run(): void
    {
        ...
        DB::transaction(function () use ($adminId, $sanitizer): void {
            foreach (self::SETTINGS as $key => $value) {
                $this->seedIdentityValue($key, $value, $adminId);
            }
...
HTML,
        ...
        $siteCache->invalidateVillageIdentity();
    ...
    private function seedIdentityValue(string $key, string $value, int $adminId): void
    {
        $column = VillageIdentity::KEY_MAP[$key] ?? null;
        if ($column === null) {
            return;
        }

        $identity = VillageIdentity::query()->firstOrNew(['id' => 1]);
        $currentValue = trim((string) $identity->getAttribute($column));
        $mayReplace = ! $identity->exists
            || $currentValue === ''
