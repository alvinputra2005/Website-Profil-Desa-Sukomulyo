...
class AdminLetterApplicationIndexQuery
{
    public function paginate(Request $request)
    {
        return LetterApplication::query()->with(['service', 'assignee'])
            ...
            ->when($request->filled('assigned_to') && $request->input('assigned_to') !== 'unassigned', fn ($q) => $q->where('assigned_to', $request->assigned_to))
            ->when($request->input('assigned_to') === 'unassigned' || $request->boolean('unassigned'), fn ($q) => $q->whereNull('assigned_to'))
