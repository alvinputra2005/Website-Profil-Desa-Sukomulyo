...
class CrudController extends Controller
{
 ...
 public function create(Request $r,string $resource){$c=$this->resource($resource);$this->ensureResourceAuthorized($c,'create');if($resource==='publications'&&$r->query('type'))$c['title']=['announcement'=>'Pengumuman Desa','agenda'=>'Agenda','document'=>'Dokumen Publik','regulation'=>'Peraturan Desa'][$r->query('type')]??$c['title'];return view($resource==='news'?'admin.news.form':'admin.crud.form',$this->formData($resource,$c,new $c['model']));}
 ...
 public function edit(string $resource,int $id){$c=$this->resource($resource);$item=$c['model']::findOrFail($id);$this->ensureResourceAuthorized($c,'update',$item);if($resource==='news')$item->load(['featuredImage','category']);if($resource==='publications')$c['title']=['announcement'=>'Pengumuman Desa','agenda'=>'Agenda','document'=>'Dokumen Publik','regulation'=>'Peraturan Desa'][$item->type]??$c['title'];return view($resource==='news'?'admin.news.form':'admin.crud.form',$this->formData($resource,$c,$item));}
