...
<div class="letter-fields">
    ...
    <label>Jenis kelamin <em>*</em><select name="sex" required><option value="" disabled @selected($value('sex') === '')>Pilih</option><option value="L" @selected($value('sex') === 'L')>Laki-laki</option><option value="P" @selected($value('sex') === 'P')>Perempuan</option></select></label>
    <label>Dusun <em>*</em><select name="hamlet" required><option value="" disabled @selected($value('hamlet') === '')>Pilih dusun</option>@foreach(($hamlets ?? ['Gumul', 'Talasan', 'Bakir', 'Kedungrejo', 'Biyan']) as $hamlet)<option value="{{ $hamlet }}" @selected($value('hamlet') === $hamlet)>{{ $hamlet }}</option>@endforeach</select></label><label>RT <em>*</em><input name="rt" value="{{ $value('rt') }}" inputmode="numeric" required></label><label>RW <em>*</em><input name="rw" value="{{ $value('rw') }}" inputmode="numeric" required></label>
