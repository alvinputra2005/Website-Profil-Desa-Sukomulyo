<nav class="news-pagination" role="navigation" aria-label="Navigasi halaman berita">
    ...
    @else
        <a class="news-pagination-arrow" href="{{ $paginator->previousPageUrl() }}#news-list" rel="prev" data-ajax data-ajax-scroll-target="#news-list">
            <i class="fas fa-chevron-left" aria-hidden="true"></i>
            <span class="screen-reader-text">Halaman sebelumnya</span>
        </a>
    @endif
        ...
        @foreach ($elements as $element)
            ...
            @if (is_array($element))
                @foreach ($element as $page => $url)
                    ...
                    @else
                        <a href="{{ $url }}#news-list" aria-label="Buka halaman {{ $page }}" data-ajax data-ajax-scroll-target="#news-list">{{ $page }}</a>
                    @endif
                @endforeach
            @endif
        @endforeach
    ...
    @if ($paginator->hasMorePages())
        <a class="news-pagination-arrow" href="{{ $paginator->nextPageUrl() }}#news-list" rel="next" data-ajax data-ajax-scroll-target="#news-list">
