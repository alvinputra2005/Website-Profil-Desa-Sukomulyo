        ...
        <div class="box-body">
            <div class="row">
                <div class="col-md-6">
                    ...
                    <div class="form-group"><label class="{{ $family->exists ? 'required' : '' }}">Kepala Keluarga</label><select name="head_resident_id" class="form-control select2" @required($family->exists)><option value="">Pilih penduduk</option>@foreach($residents as $resident)<option value="{{ $resident->id }}" @selected(old('head_resident_id',$family->head_resident_id)==$resident->id)>{{ $resident->nik }} — {{ $resident->name }}</option>@endforeach</select><p class="help-block">Hanya penduduk dengan nomor KK ini yang ditampilkan.</p></div>
                ...
                <div class="col-md-6">
                    ...
                    @if($family->exists)<div class="callout callout-info"><h4>{{ $family->members->count() }} anggota keluarga</h4><p>Hanya anggota dengan nomor KK ini yang dapat dipilih sebagai kepala keluarga. Mengganti kepala keluarga tidak menghapus anggota lama.</p></div>@else<div class="callout callout-info"><p>Simpan nomor KK terlebih dahulu, lalu tautkan penduduk melalui impor Excel atau formulir penduduk sebelum memilih kepala keluarga.</p></div>@endif
                </div>
            </div>
        </div>
