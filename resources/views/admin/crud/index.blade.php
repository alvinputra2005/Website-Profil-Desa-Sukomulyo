        ...
        <div class="box-tools">
            <form method="get">
                ...
                <div class="input-group input-group-sm" style="width:250px">
                    <input type="text" name="q" class="form-control pull-right" value="{{ request('q') }}" placeholder="Cari...">
        ...
        <div class="table-responsive">
            <table id="tabeldata" class="table table-bordered table-striped table-hover">
                <thead>
                    <tr>
                        @if ($isStatistics)
                            <th style="width:38px"><input type="checkbox" aria-label="Pilih semua dataset pada halaman ini" data-statistics-select-all></th>
                        @else
                            <th style="width:38px"><input type="checkbox" aria-label="Pilih semua data pada halaman ini" data-bulk-select-all></th>
                        @endif
                ...
                <tbody>
                    @forelse ($items as $item)
                        <tr>
                            @if ($isStatistics)
                                <td><input type="checkbox" name="ids[]" value="{{ $item->id }}" form="{{ $bulkFormId }}" aria-label="Pilih {{ $item->title }}" data-statistics-check></td>
                            @else
                                <td><input type="checkbox" name="ids[]" value="{{ $item->id }}" form="{{ $bulkFormId }}" aria-label="Pilih {{ $item->title ?? $item->name ?? $item->slug ?? 'data ini' }}" data-bulk-item></td>
                            @endif
                            <td>{{ $items->firstItem() + $loop->index }}</td>
                            @foreach ($config['columns'] as $key => $label)
                                <td>
                                    @php($value = data_get($item, $key))
                                    @if (in_array($key, ['status', 'is_active', 'is_visible', 'is_public']))
                                        <x-admin.badge :value="$value"/>
                                    @elseif ($value instanceof \Carbon\CarbonInterface)
                                        {{ $value->format('d-m-Y H:i') }}
                                    @else
                                        {{ Str::limit((string) $value, 70) }}
                                    @endif
                                </td>
                            @endforeach
                            <td class="table-actions">
                                @if ($resource === 'galleries' && $item->status === 'published')
                                    <a href="{{ route('galeri-desa') }}" target="_blank" class="btn btn-success btn-sm" title="Lihat"><i class="fa fa-eye"></i></a>
                                @endif
                                @if ($resource === 'publications' && $item->type === 'announcement' && $item->status === 'published')
                                    <a href="{{ route('announcements.show', $item->slug) }}" target="_blank" class="btn btn-success btn-sm" title="Lihat Pengumuman"><i class="fa fa-eye"></i></a>
                                @endif
                                <a href="{{ route('admin.resources.edit', [$resource, $item]) }}" class="btn bg-orange btn-sm" title="Ubah"><i class="fa fa-edit"></i></a>
                                @if ($resource === 'galleries' && $item->status !== 'archived')
                                    <form method="post" action="{{ route('admin.galleries.archive', $item) }}" data-confirm="Arsipkan galeri '{{ $item->title }}'?" data-confirm-tone="warning" data-confirm-title="Arsipkan Galeri" data-confirm-button="Ya, arsipkan">@csrf @method('patch')<button class="btn btn-default btn-sm" title="Arsipkan"><i class="fa fa-archive"></i></button></form>
                                @endif
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ count($config['columns']) + 3 }}" class="empty-state">
