...
<div class="row">
    ...
    <div class="col-md-7">
        <div class="box box-success">
            <div class="box-header with-border"><h3 class="box-title"><i class="fa fa-exchange"></i> Riwayat Mutasi</h3><div class="box-tools"><a href="{{ route('admin.inventory.mutations.create',[$category,$item]) }}" class="btn btn-success btn-sm"><i class="fa fa-plus"></i> Catat Mutasi</a></div></div>
            <div class="box-body"><div class="table-responsive"><table class="table table-striped"><thead><tr><th>Tanggal</th><th>Status / Jenis</th><th>Nilai / Penerima</th><th>Keterangan</th><th>Aksi</th></tr></thead><tbody>
                @forelse($item->mutations as $mutation)<tr><td>{{ $mutation->mutation_date->format('d-m-Y') }}</td><td><strong>{{ $mutation->asset_status }}</strong><br><small>{{ $mutation->mutation_type }}</small></td><td>@if($mutation->sale_price)Rp{{ number_format((float)$mutation->sale_price,0,',','.') }}@endif @if($mutation->recipient)<br>{{ $mutation->recipient }}@endif @if(!$mutation->sale_price&&!$mutation->recipient)—@endif</td><td>{{ $mutation->notes }}</td><td class="table-actions"><a href="{{ route('admin.inventory.mutations.edit',[$category,$item,$mutation]) }}" class="btn btn-xs btn-warning"><i class="fa fa-edit"></i></a><form method="post" action="{{ route('admin.inventory.mutations.destroy',[$category,$item,$mutation]) }}" data-confirm="Hapus catatan mutasi ini?" data-confirm-tone="danger">@csrf @method('delete')<button class="btn btn-xs btn-danger"><i class="fa fa-trash"></i></button></form></td></tr>
                @empty<tr><td colspan="5" class="text-center text-muted">Belum ada catatan mutasi.</td></tr>@endforelse
            </tbody></table></div></div>
        </div>
    </div>
</div>
