...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const residentFields = Array.from(form.querySelectorAll('[data-resident-value]'));
    const preview = form.querySelector('[data-full-name-preview]');
    const color = document.getElementById('organization_color');
    const colorPreview = form.querySelector('.official-color-preview');
    const camera = document.getElementById('photo_camera');
    const imagePreview = form.querySelector('[data-image-preview]');

    const currentSource = function () {
        return sourceInputs.find(function (input) { return input.checked; })?.value || 'external';
    };
    const updateFullName = function () {
        const prefix = document.getElementById('title_prefix').value.trim();
        const name = document.getElementById('name').value.trim();
        const suffix = document.getElementById('title_suffix').value.trim();
        preview.value = [prefix, name].filter(Boolean).join(' ') + (suffix ? ', ' + suffix : '');
    };
    const populateResident = function () {
        if (currentSource() !== 'resident') return;
        const option = residentSelect.options[residentSelect.selectedIndex];
        if (!option?.value) return;
        residentFields.forEach(function (field) {
            const key = field.dataset.residentValue;
            field.value = option.dataset[key] || '';
            if (window.jQuery && field.matches('select')) window.jQuery(field).trigger('change.select2');
        });
        updateFullName();
    };
    const updateSource = function () {
        const fromResident = currentSource() === 'resident';
        residentPicker.hidden = !fromResident;
        residentSelect.required = fromResident;
        residentFields.forEach(function (field) {
    ...
    sourceInputs.forEach(function (input) { input.addEventListener('change', updateSource); });
    residentSelect.addEventListener('change', populateResident);
    if (window.jQuery) window.jQuery(residentSelect).on('select2:select select2:clear', populateResident);
    ['name', 'title_prefix', 'title_suffix'].forEach(function (id) { document.getElementById(id).addEventListener('input', updateFullName); });
    color.addEventListener('input', function () {
        if (/^#[0-9A-Fa-f]{6}$/.test(color.value)) colorPreview.style.background = color.value;
    });
