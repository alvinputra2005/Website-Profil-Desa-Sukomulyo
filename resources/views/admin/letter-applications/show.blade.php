                        ...
                        <tbody>
                            @forelse($application->documents as $document)
                                <tr>
                                    ...
                                    <td><a class="document-name btn btn-link" href="{{ route('admin.letter-applications.document.preview-content', [$application, $document->id]) }}" target="_blank" rel="noopener noreferrer" data-document-preview data-preview-url="{{ route('admin.letter-applications.document.preview-url', [$application, $document->id]) }}" data-download-url="{{ route('admin.letter-applications.document', [$application, $document->id]) }}" data-document-name="{{ $document->original_name }}" data-document-size="{{ number_format(($document->size_bytes ?: $document->file_size) / 1024, 0) }} KB">{{ $document->original_name }}</a><span class="document-meta">{{ number_format(($document->size_bytes ?: $document->file_size) / 1024, 0) }} KB · Klik untuk pratinjau</span></td>
