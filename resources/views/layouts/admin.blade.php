    ...
    <div class="wrapper">
        ...
        <aside class="main-sidebar">
            <section class="sidebar">
                ...
                ['label'=>'Info Desa','icon'=>'fa-home','ability'=>'manage-content','items'=>[['Identitas Desa','admin.resources.index','profiles'],['Pemerintah Desa','admin.resources.index','officials']]],
                ...
                <ul class="sidebar-menu" data-widget="tree">
                    <li class="header">MENU UTAMA</li>@foreach($groups as $group)@if(!$group['ability']||auth()->user()->can($group['ability']))@php($active=collect($group['items'])->contains(fn($i)=>request()->routeIs($i[1])&&(!$i[2]||request()->route('resource')===$i[2])))@if($group['label']==='Beranda')<li class="{{ $active?'active':'' }}"><a href="{{ route('admin.dashboard') }}"><i class="fa {{ $group['icon'] }} {{ $active?'text-aqua':'' }}"></i><span>Dashboard</span></a></li>@else<li class="treeview {{ $active?'active menu-open':'' }}"><a href="#"><i class="fa {{ $group['icon'] }} {{ $active?'text-aqua':'' }}"></i><span>{{ $group['label'] }}</span><span class="pull-right-container"><i class="fa fa-angle-left pull-right"></i></span></a>
                        <ul class="treeview-menu">@foreach($group['items'] as [$label,$route,$resource])<li class="{{ request()->routeIs($route)&&(!$resource||request()->route('resource')===$resource)?'active':'' }}"><a href="{{ route($route,$resource) }}"><i class="fa fa-circle-o"></i>{{ $label }}</a></li>@endforeach</ul>
