...
const initAdminPage = () => {
  ...
  if(window.jQuery){const $=window.jQuery;
    ...
    $('.select2:not(.select2-hidden-accessible):not([data-select2-disabled])').select2({width:'100%'});
  ...
  const attachmentManager=document.querySelector('[data-publication-attachments]');
  if(attachmentManager){
    const fileInput=attachmentManager.querySelector('[data-attachment-files]');
    const list=attachmentManager.querySelector('[data-attachment-list]');
    let selectedFiles=[];

    const rows=()=>Array.from(list.querySelectorAll('[data-attachment-row]'));
    const syncOrder=()=>{
    ...
    fileInput.addEventListener('change',()=>{
      ...
      if(invalid||oversized||selectedFiles.length+incoming.length>10){
        const message=invalid?'Hanya file PDF yang dapat dilampirkan.':oversized?'Ukuran setiap PDF maksimal 10 MB.':'Maksimal 10 PDF per sekali simpan.';
